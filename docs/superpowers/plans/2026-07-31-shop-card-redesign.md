# ShopCard Component Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign `ShopCard.tsx` with enhanced hero banner height (`h-44`), glassmorphism verified & location pills, refined logo placement, permit micro-badges, and interactive CTA hover animations.

**Architecture:** Update `resources/js/components/ShopCard.tsx` following modern UI design principles, responsive layout math, and accessibility standards.

**Tech Stack:** React 19, Tailwind CSS v4, Lucide React icons, TypeScript.

## Global Constraints

- Preserve all existing props (`shop: Shop`, `productCount: number`) and WhatsApp event handlers.
- Maintain responsive layout with zero layout shifts.
- Ensure proper accessibility attributes (`aria-label`, `aria-hidden`, semantic `<article>`).

---

### Task 1: Redesign `ShopCard.tsx` Component

**Files:**
- Modify: `resources/js/components/ShopCard.tsx`

**Interfaces:**
- Consumes: `{ shop: Shop; productCount: number }`
- Produces: Redesigned `ShopCard` component.

- [ ] **Step 1: Replace `resources/js/components/ShopCard.tsx` with redesigned code**
...
- [ ] **Step 2: Verify Vite build and Pest test suite**

Run: `npm run build`
Run: `./vendor/bin/pest --filter=MarketplaceControllerTest`

- [ ] **Step 3: Commit redesigned ShopCard**

```bash
git add resources/js/components/ShopCard.tsx
git commit -m "refactor: redesign ShopCard component with enhanced glassmorphism badges and motion"
```
