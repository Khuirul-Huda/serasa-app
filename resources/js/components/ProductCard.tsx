...
export default function ProductCard({ product, shop }: ProductCardProps) {
    return (
        <article
            className="group flex h-full flex-col overflow-hidden rounded-2xl border border-navy-200/60 bg-white font-sans shadow-2xs transition-all duration-300 hover:-translate-y-0.5 hover:border-pastel-teal hover:shadow-md dark:border-navy-800 dark:bg-navy-900/90"
            id={`product-card-${product.id}`}
        >
            <Link
                href={`/products/${product.id}`}
                className="relative block aspect-square overflow-hidden bg-navy-50 dark:bg-navy-950"
            >
            ...
            <div className="flex flex-1 flex-col justify-between space-y-3 bg-white p-3.5 sm:p-4 dark:bg-navy-900/90">
                <div className="space-y-1.5">
                    {shop && (
                        <div className="flex min-w-0 items-center gap-1.5">
                            <img
                                ...
                                height={20}
                                loading="lazy"
                                className="h-5 w-5 shrink-0 rounded-full border border-navy-200 object-cover dark:border-navy-700"
                                referrerPolicy="no-referrer"
                            />
                            <Link
                                href={`/shops/${shop.id}`}
                            ...
                            >
                                ...
                                {shop.isVerified && (
                                    <CheckCircle2
                                        className="h-3.5 w-3.5 shrink-0 fill-pastel-teal-light text-pastel-teal dark:fill-navy-800"
                    ...
                    <h3 className="line-clamp-2 min-h-[38px] pt-0.5 font-sans text-sm leading-snug font-bold text-navy-800 transition-colors group-hover:text-pastel-teal md:text-base dark:text-navy-100 dark:group-hover:text-pastel-teal">
                    ...
                    <div className="pt-0.5">
                        <div className="flex flex-wrap items-baseline gap-1.5">
                            <data
                                value={product.price}
                                className="text-base font-black text-navy-900 md:text-lg dark:text-white"
                            >
